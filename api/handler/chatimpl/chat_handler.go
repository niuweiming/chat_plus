...
// ChatHandle 是处理WebSocket聊天连接的函数。
func (h *ChatHandler) ChatHandle(c *gin.Context) {
	...
	// 根据升级后的连接创建WebSocket客户端实例。
	client := types.NewWsClient(ws)

	// 根据提供的模型ID查询数据库中的聊天模型信息。
	var chatModel model.ChatModel
	res := h.DB.First(&chatModel, modelId)
	if res.Error != nil || chatModel.Enabled == false { // 如果找不到模型或模型未启用，
		utils.ReplyMessage(client, "当前AI模型暂未启用，连接已关闭！！！") // 向客户端发送错误消息并终止连接。
		c.Abort()
		return
	}
	...
	logger.Infof("新WebSocket连接已建立，IP: %s，用户名: %s", c.ClientIP(), session.Username)
	...
	go func() {
		for {
			...
			if message.Type == "heartbeat" {
				logger.Debug("收到心跳消息：", message.Content)
				continue
			}

			// 记录接收到的消息。
			logger.Error("接收到消息: ", message.Content)
			...
			if err != nil {
				logger.Error(err)                                                   // 应该是记录错误而非判断err==nil
				utils.ReplyChunkMessage(client, types.WsMessage{Type: types.WsEnd}) // 发送结束消息。
			} else {
				utils.ReplyChunkMessage(client, types.WsMessage{Type: types.WsEnd}) // 正常情况下也发送结束消息。
				logger.Infof("回答完毕: %v", message.Content)                           // 记录回答完成日志。
			}
		}
	}()
}
...
func (h *ChatHandler) sendMessage(ctx context.Context, session *types.ChatSession, role model.ChatRole, prompt string, ws *types.WsClient) error {
	if session == nil || prompt == "" {
		logger.Error("传输参数不完整")
	}
	if !h.App.Debug {
		defer func() {
			if r := recover(); r != nil {
...
// 记录用户是否满意
func (h *ChatHandler) ReviewUser(c *gin.Context) {
	logger.Error("执行ReviewUser函数")
	userReview := &vo.Review{}
	if err := c.ShouldBindJSON(userReview); err != nil {
		logger.Error("用户满意度表参数绑定失败!")
		logger.Error(err)
	}
	modelId := h.GetInt(c, "model_id", 0)
	reviewUser := model.Reviews{
	...
	if err := h.DB.Create(&reviewUser).Error; err != nil {
