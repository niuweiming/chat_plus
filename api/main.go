...
func main() {
	...
	app := fx.New(
		...
		fx.Provide(chatimpl.NewChatBotHandler),
		...
		fx.Provide(admin.NewChatBotmailbox),
		...
		fx.Invoke(func(s *core.AppServer, h *chatimpl.ChatHandler) {
			...
			group.POST("review", h.ReviewUser)
		}),
		...
		//QAny机器人接口调用
		fx.Invoke(func(s *core.AppServer, h *chatimpl.ChatBotHandler) {
		...
		//机器人邮件箱
		fx.Invoke(func(s *core.AppServer, h *admin.ChatBotmailbox) {
			group := s.Engine.Group("/api/admin/mailbox/")
			group.GET("search", h.List)
			group.POST("reply", h.Upload)
		}),
